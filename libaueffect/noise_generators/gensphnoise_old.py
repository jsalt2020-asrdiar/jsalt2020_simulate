...
class SphericalNoiseGenerator(object):
    def __init__(self, sound_velocity=340, fs=16000, micarray='circular7', spectral_shape='hoth'):
    ...
    def __call__(self, nsamples, micarray=None):
        if micarray is None:
            return libaueffect.noise_generators.functions.generate_isotropic_noise(self._micarray, nsamples, self._fs, type='sph', spectrum=self._spectral_shape)
        else:
            return libaueffect.noise_generators.functions.generate_isotropic_noise(micarray, nsamples, self._fs, type='sph', spectrum=self._spectral_shape)
